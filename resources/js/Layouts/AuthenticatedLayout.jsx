                    ...
                    <nav className="sticky top-0 z-50 bg-slate-900/90 backdrop-blur-md border-b border-white/10">
                            ...
                            <div className="flex items-center justify-between h-16">
                                ...
                                <div className="flex-1 flex items-center justify-center">
                                    <div className="hidden sm:flex space-x-6">
                                        ...
                                        <Link href={route('ai.assistant')} className={makeLinkClass(route().current('ai.assistant'))}>Spark</Link>
                        ...
                        <div className={(showingNavigationDropdown ? 'block' : 'hidden') + ' sm:hidden bg-slate-900/90 backdrop-blur-md'}>
                            <div className="pt-2 pb-3 space-y-1">
                                <Link href={route('dashboard')} className={makeLinkClass(route().current('dashboard')) + ' block pl-4'}>Dashboard</Link>
                                <Link href={route('projects.index')} className={makeLinkClass(route().current('projects.*')) + ' block pl-4'}>Projects</Link>
                                <Link href={route('chat')} className={makeLinkClass(route().current('chat')) + ' block pl-4'}>Chat</Link>
                                <Link href={route('ai.assistant')} className={makeLinkClass(route().current('ai.assistant')) + ' block pl-4'}>Spark</Link>
                                <Link href={route('projects.create')} className={makeLinkClass(route().current('projects.create')) + ' block pl-4'}>Upload</Link>
                            </div>
